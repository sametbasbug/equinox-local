...
export function scheduleEquinoxLocalRestart({
  installation,
  ...
  const child = spawnImpl(nodePath, [helperPath, "--restart"], {
    detached: true,
    stdio: "ignore",
    env: restartHelperEnvironment(installation, sourceEnv),
  });
  if (!child || typeof child.unref !== "function") {
    throw new Error("Equinox Local restart helper failed to start.");
  }
  child.unref();
