...
async function fakeInstall(releaseDir) {
  ...
  await fs.writeFile(path.join(destination, "VERSION"), "4.5.0\n", { mode: 0o600 });
}
...
test("source Peekaboo sync installs and pins the managed developer runtime", async (t) => {
  ...
  assert.deepEqual(result, { changed: true, version: "4.5.0" });
  ...
  assert.equal(managedLine.includes("Equinox Local Developer/runtime/peekaboo/versions/4.5.0/peekaboo"), true);
});
...
test("source Peekaboo sync is a no-op once the pinned runtime is healthy", async (t) => {
  ...
  assert.deepEqual(result, { changed: false, version: "4.5.0" });
