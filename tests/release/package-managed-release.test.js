...
test("pinned tunnel runtime metadata covers both supported macOS architectures", () => {
  assert.equal(EQUINOX_LOCAL_TUNNEL_CLIENT_VERSION, "0.0.12");
  ...
  assert.equal(TUNNEL_CLIENT_DISTRIBUTIONS["darwin-arm64"].filename, "tunnel-client-v0.0.12-darwin-arm64.zip");
  assert.equal(TUNNEL_CLIENT_DISTRIBUTIONS["darwin-x64"].filename, "tunnel-client-v0.0.12-darwin-amd64.zip");
});
