...
test("public shell installer stays user-level, pinned to Equinox HTTPS and bounded", async () => {
  ...
  assert.match(source, /installation is ready; Equinox Local is starting through its LaunchAgent/u);
  assert.doesNotMatch(source, /CONTROL_CENTER_APP|CONTROL_CENTER_EXECUTABLE|NATIVE_APP_PID|NATIVE_APP_OPENED/u);
  assert.doesNotMatch(source, /\/usr\/bin\/nohup/u);
  assert.doesNotMatch(source, /\/usr\/bin\/open(?:\s|-)/u);
